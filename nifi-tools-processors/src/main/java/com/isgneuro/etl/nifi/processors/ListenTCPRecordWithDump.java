...
@WritesAttributes({ @WritesAttribute(attribute = "tcp.sender", description = "The host that sent the data."),
...
public class ListenTCPRecordWithDump extends AbstractProcessor {

	static final PropertyDescriptor PORT = new PropertyDescriptor.Builder().name("port").displayName("Port")
			.description("The port to listen on for communication.").required(true)
	...
	static final PropertyDescriptor READ_TIMEOUT = new PropertyDescriptor.Builder().name("read-timeout")
			...
			.addValidator(StandardValidators.TIME_PERIOD_VALIDATOR).defaultValue("10 seconds").required(true).build();

	static final PropertyDescriptor MAX_SOCKET_BUFFER_SIZE = new PropertyDescriptor.Builder()
			.name("max-size-socket-buffer").displayName("Max Size of Socket Buffer")
			.description(
					"The maximum size of the socket buffer that should be used. This is a suggestion to the Operating System "
			...
			.addValidator(StandardValidators.DATA_SIZE_VALIDATOR).defaultValue("1 MB").required(true).build();

	static final PropertyDescriptor MAX_CONNECTIONS = new PropertyDescriptor.Builder()
			.name("max-number-tcp-connections").displayName("Max Number of TCP Connections")
			.description(
					"The maximum number of concurrent TCP connections to accept. In cases where clients are keeping a connection open, "
			...
			.addValidator(StandardValidators.createLongValidator(1, 65535, true)).defaultValue("2").required(true)
			.build();
	...
	static final PropertyDescriptor WRITE_DUMP = new PropertyDescriptor.Builder().name("write-dump")
			...
			.description(
					"If set to 'true', dump file will be created. If 'false', processor functionality is similar to ListenTCPRecord processor.")
			.required(true).defaultValue("false").allowableValues(new String[] { "true", "false" }).build();

	static final PropertyDescriptor PATH_TO_DUMP = new PropertyDescriptor.Builder().name("path-to-dump")
			.displayName("Path to dump").description("Path to file with dump.").required(false)
			.addValidator(StandardValidators.URI_VALIDATOR)
			.expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY).build();

	static final PropertyDescriptor WRITE_MANY = new PropertyDescriptor.Builder().name("write-many")
			.displayName("Write many")
			.description(
					"If set to 'true', many files will be written by each node. Otherwise will be written one file by one node.")
			.required(false).defaultValue("false").allowableValues(new String[] { "true", "false" }).build();

	static final AllowableValue ERROR_HANDLING_DISCARD = new AllowableValue("Discard", "Discard",
			"Discards any records already received and closes the connection.");
	static final AllowableValue ERROR_HANDLING_TRANSFER = new AllowableValue("Transfer", "Transfer",
			"Transfers any records already received and closes the connection.");

	static final PropertyDescriptor READER_ERROR_HANDLING_STRATEGY = new PropertyDescriptor.Builder()
			.name("reader-error-handling-strategy").displayName("Read Error Strategy")
			.description(
			...
			.required(true).allowableValues(ERROR_HANDLING_TRANSFER, ERROR_HANDLING_DISCARD)
			.defaultValue(ERROR_HANDLING_TRANSFER.getValue()).build();
	...
	static final PropertyDescriptor TIME_WINDOW = new PropertyDescriptor.Builder().displayName("Time window")
			...
			.addValidator(StandardValidators.TIME_PERIOD_VALIDATOR).required(true).defaultValue("1 hour").build();
	...
	static final PropertyDescriptor CLIENT_AUTH = new PropertyDescriptor.Builder().name("client-auth")
			.displayName("Client Auth")
			.description(
					"The client authentication policy to use for the SSL Context. Only used if an SSL Context Service is provided.")
			.required(false).allowableValues(ClientAuth.values()).defaultValue(ClientAuth.REQUIRED.name()).build();

	static final Relationship REL_SUCCESS = new Relationship.Builder().name("success")
			.description("Messages received successfully will be sent out this relationship.").build();
	...
	static {
		...
		props.add(TIME_WINDOW);
		props.add(SSL_CONTEXT_SERVICE);
		props.add(CLIENT_AUTH);
		props.add(WRITE_DUMP);
		props.add(PATH_TO_DUMP);
		props.add(WRITE_MANY);
	...
	protected Collection<ValidationResult> customValidate(final ValidationContext validationContext) {
		...
		final SSLContextService sslContextService = validationContext.getProperty(SSL_CONTEXT_SERVICE)
				.asControllerService(SSLContextService.class);

		if (sslContextService != null && StringUtils.isBlank(clientAuth)) {
			results.add(new ValidationResult.Builder().explanation("Client Auth must be provided when using TLS/SSL")
	...
	public void onScheduled(final ProcessContext context) throws IOException {
		...
		// if the Network Interface Property wasn't provided then a null InetAddress
		// will indicate to bind to all interfaces
		...
		// create a ServerSocketChannel in non-blocking mode and bind to the given
		// address and port
	...
	public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
		...
		final long timeWindow = context.getProperty(TIME_WINDOW).asTimePeriod(TimeUnit.MILLISECONDS);
		...
		final String pathToDump = context.getProperty(PATH_TO_DUMP).getValue();
		Boolean writeDumpProperty = context.getProperty(WRITE_DUMP).asBoolean();
		final Boolean writeDump = writeDumpProperty && pathToDump != null && !pathToDump.equals("");
		final Boolean writeMany = context.getProperty(WRITE_MANY).asBoolean();
		...
		synchronized (socketRecordReader) {
			...
			try {
				// lazily creating the record reader here b/c we need a flow file, eventually
				...
				try {
					...
					// some of the underlying record libraries wrap the real exception in
					// RuntimeException, so check each
					// throwable (starting with the current one) to see if its a
				...
				String nodeHostname = InetAddress.getLocalHost().getHostName();
				if (!new File(pathToDump).exists())
					new File(pathToDump).mkdir();
				String path = pathToDump + "/" + nodeHostname;
				if (writeMany) {
					path += "/" + UUID.randomUUID();
					if (!new File(pathToDump + "/" + nodeHostname).exists())
				...
				try (final OutputStream out = session.write(flowFile);
						final RecordSetWriter recordWriter = recordSetWriterFactory.createWriter(getLogger(),
								recordSchema, out, flowFile);
						final Writer fileWriter = writeDump
								? new BufferedWriter(new FileWriter(path, new File(path).exists()))
								: null) {
					...
					if (writeDump)
						fileWriter.write(record.getAsString("_raw") + "\n");

					StopWatch sw = new StopWatch(true);
					while (record != null && writeResult.getRecordCount() < recordBatchSize
							&& sw.getElapsed(TimeUnit.MILLISECONDS) < timeWindow) {
						// handle a read failure according to the strategy selected...
						// if discarding then bounce to the outer catch block which will close the
						// connection and remove the flow file
						// if keeping then null out the record to break out of the loop, which will
						// transfer what we have and close the connection
						try {
							record = recordReader.nextRecord();
